...
void renderQuad();
void renderWall();
void renderBloomScene(Shader &shader, vector<glm::vec3>&, vector<glm::vec3>&);
...
int main() {
	//initial and configure
	glfwInit();
	glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR,3);
	glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR,3);
	glfwWindowHint(GLFW_OPENGL_PROFILE,GLFW_OPENGL_CORE_PROFILE);

	GLFWwindow* window = glfwCreateWindow(WIDTH,HEIGHT,"first",NULL, NULL);
	if (window == NULL) {
		cout << "what fuck" << endl;
		glfwTerminate();
		return -1;
