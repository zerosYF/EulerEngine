...
namespace EulerEngine {
	EulerShader::EulerShader(const std::string& vertexSrc, const std::string& fragmentSrc) {
	...
	EulerShader::~EulerShader() {
		glDeleteProgram(m_RendererID); 
	}
	void EulerShader::CompileShader(const char* Code, unsigned int& shader, CompileShaderType type) {
	...
	void EulerShader::CheckError(unsigned int object,unsigned int type) {
		int success;
		char infoLog[512];
